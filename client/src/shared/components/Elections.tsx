...
import type {Election as ElectionApiModel, ElectionStatus, ElectionSummary} from "../types/election";
...
const STATUS_LABEL_KEYS = {
    upcoming: "list.status.upcoming",
    open: "list.status.open",
    closed: "list.status.closed",
} as const satisfies Record<ElectionStatus, string>;

const STATUS_BADGE_CLASSES: Record<ElectionStatus, string> = {
...
const Elections: React.FC = () => {
    ...
    const resolveLifecycleError = useCallback(
        (error: unknown) => {
            ...
            if (/election cannot be started before.*configured start\s*time/i.test(message)) {
                return t("list.messages.startBeforeConfiguredStartTime", {
    ...
    const handleDeploy = async (election: ElectionSummary) => {
        ...
        try {
            ...
            const message = resolveLifecycleError(error);
    ...
    const handleEndElection = async () => {
        ...
        try {
            ...
            setDialogError(resolveLifecycleError(error));
        } finally {
        ...
        content = (
            <section className="elections-grid" aria-label={t("list.title")}>
                {filtered.map((election) => {
                    ...
                    const voteButtonLabel = status === "open" && election.hasVoted ? t("list.voted") : t("list.vote");

                    let adminActionButton: React.ReactNode = null;
                    if (isAdmin && canDeploy) {
                        adminActionButton = (
                            <button
                    ...
                    } else if (isAdmin && election.contractAddress && election.phase === "REGISTRATION") {
                        adminActionButton = (
                            <button
                                className="secondary-button"
                                disabled={actionBusyId === election.id}
                    ...
                    } else if (isAdmin && election.contractAddress && election.phase === "VOTING") {
                        adminActionButton = (
                            <button
                                className="secondary-button"
                                disabled={actionBusyId === election.id}
        ...
        <div className="elections-page">
            ...
            {endDialogElection ? (
                <div className="election-dialog-backdrop">
                    <dialog className="election-dialog" open aria-labelledby="election-dialog-title">
                        ...
                        <div className="election-dialog-actions">
                            <button className="secondary-button" onClick={closeEndDialog}
                                    disabled={actionBusyId === endDialogElection.id}>
                                {t("list.dialog.cancel")}
                            </button>
                            <button className="primary-button" onClick={() => void handleEndElection()}
                                    disabled={actionBusyId === endDialogElection.id}>
